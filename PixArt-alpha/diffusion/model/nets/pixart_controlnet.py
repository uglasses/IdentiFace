...
class ControlPixArtHalf(Module):
    ...
    def __init__(
        ...
        super().__init__()
        if use_vae_dual_fusion or use_dual_nonlinear_control:
            raise ValueError(
                "VaeLatentFusion3x3 and DualNonlinearControlFusion have been removed; "
                "use FrequencyControlFusion only."
            )
    ...
    def _prepare_control_latent(self, c: Tensor, c2: Optional[Tensor], timestep: Optional[Tensor]) -> Tensor:
        ...
        if c2 is not None:
            raise ValueError(
                "c2 was passed but model was built without FrequencyControlFusion."
            )
        return c
    ...
    def load_state_dict(self, state_dict: Mapping[str, Any], strict: bool = True):
        # New ControlNet checkpoints may contain base_model/controlnet/freq_fusion.
        # Old checkpoints may be an unwrapped base model (blocks.N.xxx; remap to blocks.N.base_block.xxx).
        if all(
            k.startswith(("base_model", "controlnet", "freq_fusion"))
...
class ControlPixArtMSHalf(ControlPixArtHalf):
    ...
    def forward(self, x, timestep, y, mask=None, data_info=None, c=None, c2=None, **kwargs):
        ...
        t: (N,) tensor of diffusion timesteps
        y: (N, 1, 120, C) tensor of class labels
        c2: optional second 4ch VAE control latent (LQ) when FrequencyControlFusion is enabled.
        """
        if c is not None:
            c = c.to(self.dtype)
            if c2 is not None:
