...
class HEDdetector(nn.Module):
    def __init__(self, feature=True, vae=None, hed_model_path=None):
        super().__init__()
        self.model = ControlNetHED_Apache2()
        
        # Try multiple possible HED model paths
        if hed_model_path is None:
            # Default path list (by priority)
            possible_paths = [
                'models/ControlNetHED.pth',
                'output/pretrained_models/ControlNetHED.pth',
                os.path.join(os.path.dirname(__file__), '../../models/ControlNetHED.pth'),
                os.path.join(os.path.dirname(__file__), '../../../models/ControlNetHED.pth'),
                '/home/jovyan/maao-data-cephfs-0/dataspace/maao/projects/Common/models/PixArt-alpha/ControlNetHED.pth',
            ]
            ...
            for path in possible_paths:
                ...
                if os.path.exists(abs_path):
                    hed_model_path = abs_path
                    print(f"[INFO] Found HED model at: {hed_model_path}")
                    break
        
        if hed_model_path is None or not os.path.exists(hed_model_path):
            raise FileNotFoundError(
                f"ControlNetHED.pth not found. Please download it from:\n"
                f"  https://huggingface.co/PixArt-alpha/PixArt-alpha/resolve/main/ControlNetHED.pth\n"
                f"  And place it in one of these locations:\n"
                f"  - models/ControlNetHED.pth\n"
                f"  - output/pretrained_models/ControlNetHED.pth\n"
                f"  Or specify the path using hed_model_path parameter."
            )
        
        self.model.load_state_dict(torch.load(hed_model_path, map_location='cpu'))
        self.model.eval()
