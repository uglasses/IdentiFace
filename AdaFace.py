...
if os.path.isdir(ADAFACE_DIR) and ADAFACE_DIR not in sys.path:
    sys.path.insert(0, ADAFACE_DIR)  # Use insert(0) to ensure this path is searched first
...
def _gallery_relpath_key(path: str, gallery_dir: str) -> str:
    """Stable cache key: path relative to gallery_dir (survives parent-dir rename).

    ``.../dataset_new/CelebA/CelebA-HQ-img/0.jpg`` and
    ``.../dataset/ID-CelebA/CelebA-HQ-img/0.jpg`` both become ``0.jpg``.
    """
    if not path or not gallery_dir:
        return os.path.basename(str(path))
    path_norm = str(path).replace("\\", "/")
    gallery_real = os.path.realpath(os.path.abspath(gallery_dir))
    gallery_name = os.path.basename(gallery_real.rstrip("/\\"))
    abs_candidate = os.path.realpath(path_norm) if os.path.exists(path_norm) else os.path.normpath(path_norm)
    try:
        rel = os.path.relpath(abs_candidate, gallery_real).replace("\\", "/")
        if rel == ".":
        ...
        pass
    marker = f"/{gallery_name}/"
    if marker in path_norm:
        return path_norm.split(marker, 1)[-1]
    return os.path.basename(path_norm)
...
def _get_or_extract_candidate_embeddings(
    ...
    If gallery_dir is set, cache keys are paths relative to that directory
    (so moving/renaming parent folders does not miss). Returned paths stay
    the original candidate_paths (absolute).
    """
    path_to_feat_cached, loaded = _load_embedding_cache(cache_file)
    use_rel = bool(gallery_dir)
    keys_need_rewrite = False
    if use_rel and path_to_feat_cached:
        remapped = {}
    ...
    for p in candidate_paths:
        lookup = _gallery_relpath_key(p, gallery_dir) if use_rel else p
    ...
    if missing_paths:
        ...
        for p, feat in zip(missing_valid_paths, missing_feats):
            store_key = _gallery_relpath_key(p, gallery_dir) if use_rel else p
            path_to_feat_cached[store_key] = feat
            out_paths_valid.append(p)
    ...
    if loaded:
        if missing_paths or keys_need_rewrite:
    ...
    if running_inputs_batch:
        ...
        if args.candidates_dir:
            ...
            cache_file = os.path.join(
                ...
                "adaface_emb_fefc287370e30c28.npz",
            )
